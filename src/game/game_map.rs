use super::{game_constants::MAX_N_TILES_IN_WHOLE_MAP, mapchunk::MapChunk};
...
pub const MAP_TILESETS: [[u8; 16]; 9] = [
    [
        // normal
        0, 20, 0, 17, 0, 18, 0, 19, 0, 12, 9, 10, 11, 12, 13, 14,
    ],
    [
        // oblong
        0, 20, 10, 9, 13, 18, 11, 9, 8, 9, 9, 10, 11, 12, 13, 14,
    ],
    [
        // teeny tunnels
        0, 9, 27, 9, 29, 9, 28, 9, 25, 9, 9, 10, 11, 12, 13, 14,
    ],
    [
        // large normal
        0, 12, 27, 12, 29, 12, 28, 12, 25, 30, 9, 10, 11, 32, 31, 14,
    ],
    [
        // small (jungle vibes)
        0, 12, 27, 14, 29, 12, 28, 14, 25, 12, 9, 31, 32, 32, 31, 32,
    ],
    // for clouds stage use pillars and platforms! :D
    [
        // rediculous tunnels
        0, 12, 10, 12, 13, 12, 11, 12, 8, 9, 9, 9, 31, 9, 32, 32,
    ],
    [
        // chaotic
        0, 20, 0, 30, 0, 18, 0, 30, 0, 30, 31, 27, 29, 25, 28, 32,
    ],
    [
        // spacey
        0, 20, 10, 15, 13, 12, 11, 15, 8, 12, 12, 10, 11, 8, 13, 12,
    ],
    [
        // underworld
        26, 32, 26, 32, 26, 32, 26, 32, 0, 26, 25, 27, 29, 31, 28, 28,
    ],
];
    ...
    pub fn link_chunk_to_touching_chunks(&mut self, chunk: &mut MapChunk) {
        for other_chunk in self.chunks.iter_mut() {
            fn fuse_horizontal(chunk: &mut MapChunk, other_chunk: &mut MapChunk) {
                ...
                if other_chunk.bound.y + other_chunk.bound.height as i32 == chunk.bound.y
                    ...
                    && other_chunk.bound.x < chunk.bound.x + chunk.bound.width as i32
                {
                    ...
                    let max_x = core::cmp::min(
                        chunk.bound.x + chunk.bound.width as i32,
                        other_chunk.bound.x + other_chunk.bound.width as i32,
                    );
            ...
            fn fuse_vertical(chunk: &mut MapChunk, other_chunk: &mut MapChunk) {
                // check to see if the other chunk touches the left of the new chunk
                if other_chunk.bound.x + other_chunk.bound.width as i32 == chunk.bound.x
                    ...
                    && other_chunk.bound.y < chunk.bound.y + chunk.bound.height as i32
                {
                    ...
                    let max_y = core::cmp::min(
                        chunk.bound.y + chunk.bound.height as i32,
                        other_chunk.bound.y + other_chunk.bound.height as i32,
                    );
                    for absolute_coord_y in min_y..max_y {
                        ...
                        if rel_other_chunk_y == 0
                            || rel_other_chunk_y == other_chunk.bound.height as i32 - 1
                            || rel_chunk_y == 0
                            || rel_chunk_y == chunk.bound.height as i32 - 1
                        {
